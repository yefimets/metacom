...
test('Chunk / chunkEncode', async (t) => {
  ...
  await t.test('handles single character ID', () => {
    const id = 'a';
  ...
  await t.test('handles UTF-8 characters', () => {
  ...
  await t.test('throws error for ID exceeding 255 bytes', () => {
...
test('Chunk / chunkDecode', async (t) => {
  await t.test('decodes chunk back to ID and payload', () => {
  ...
  await t.test('handles maximum ID length', () => {
...
test('Chunk / encode-decode', async (t) => {
  await t.test('round-trip with various payloads', () => {
    const testCases = [
  ...
  await t.test('preserves binary data integrity', () => {
