...
/// the phone: `@Name text` is a command to that agent, `@auto text` lets the hub pick,
/// anything else is posted to the room. Only the owner's messages are relayed inwards;
...
const parse = (text) => {
  ...
  if (target.toLowerCase() === 'auto') return { kind: 'dispatch', text: rest };
  if (target.toLowerCase() === 'room') return { kind: 'say', text: rest };
  return { kind: 'command', to: target, text: rest };
};

/// A room message as one Telegram line. Null for what the group should not see.
const format = (m) => {
...
class Telegram {
  constructor({ hub, console, botToken, owner = '', dataDir, fetchImpl = fetch, api = API }) {
    this.hub = hub;
  ...
  start() {
    ...
    this.hub.events.on('room/message', (m) => this.onRoomMessage(m));
    this.hub.events.on('agents/attention', (member) => this.onAttention(member));
  ...
  async onUpdate(msg) {
    ...
    const conn = this.hub.local(`tg:${msg.from.username || msg.from.first_name || msg.from.id}`);
    if (action.kind === 'say') {
      this.hub.say(conn, bound.room, action.text);
    } else if (action.kind === 'dispatch') {
      const r = await this.hub.dispatch(conn, action.text, bound.room);
      await this.reply(chatId, `→ ${r.agent} (${r.reason})${r.delivered ? '' : ', queued'}`);
    } else {
      const r = await this.hub.send(conn, action.to, action.text, 'command');
  ...
  async slash(chatId, chat, from, { name, arg }) {
    ...
    if (name === 'start' || name === 'help') {
      return this.reply(chatId, 'metacom. /join <room> binds this group to a room, /leave unbinds, /agents lists them, /read <agent> shows its screen. Then: "@Name text" commands an agent, "@auto text" lets the hub pick, anything else goes to the room.');
    }
    if (name === 'join') {
    ...
    const conn = this.hub.local('tg:' + (from.username || from.id));
    if (name === 'leave') {
    ...
    if (name === 'agents') {
      const list = this.hub.list(conn, bound.room).filter((m) => m.kind === 'agent');
    ...
    if (name === 'read') {
      ...
      const r = await this.hub.read(conn, agent, 40);
      return this.reply(chatId, '```\n' + (r.text || '(empty)').slice(-MAX_MESSAGE + 10) + '\n```', 'Markdown');
    }
    return undefined;
  }
