...
/// stdio MCP server spawned by the agent (claude --mcp-config). It joins metacom as the same
...
const serveMcp = async (config) => {
  ...
  const local = async (raw) => {
    const msg = { ...raw, text: await mc.rooms.open(raw.room || room, raw.text) };
  ...
  // the room a member lives in decides which key a directed message uses
  const roomOf = async (who) => (await mc.api.agents.list({})).find((m) => m.name === who)?.room || room;
  ...
  // MC_PROMPT is the room briefing the wrapper built. Claude Code also gets it as a system
  // prompt and opencode as an instructions file; carrying it here too means a harness that
  // only knows MCP still learns what the [metacom …] lines in its terminal are.
  const instructions = process.env.MC_PROMPT || undefined;
  const server = new McpServer({ name: 'metacom', version: '0.1.0' }, instructions ? { instructions } : {});

  server.tool('mc_agents', 'List the agents and humans on metacom with status (working, waiting, stopped), room, host, repo, and whose commands each agent accepts (owner, any, or names): you can mc_send a command to the ones that accept you.', {}, async () => {
  ...
  server.tool(
    ...
    async ({ text: body }) => {
      const msg = await mc.api.room.say({ room, text: await mc.rooms.close(room, body) });
  ...
  server.tool(
    ...
    async ({ to, text: body, kind }) => {
      const result = await mc.api.agents.send({ to, text: await mc.rooms.close(await roomOf(to), body), kind: kind || 'command' });
