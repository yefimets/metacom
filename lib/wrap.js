...
const { configure, kindOf } = require('./harness.js');
...
// counts as waiting. Long enough to outlast a spinner frame, short enough to feel immediate.
const SETTLE_MS = 1200;
// A frame of a repainting TUI: clear, redraw. Bytes pause for longer than this between frames.
...
/// Where the wrapper keeps files of its own: the debug trace, a harness briefing file.
const stateDir = () => {
...
const wrap = async ({ name, room, repo, caps, accept, command, args, config, mcp = true }) => {
  ...
  // claude, codex and opencode each take the room's MCP server and briefing their own way
  const isClaude = kindOf(command) === 'claude';
  ...
  const setup = configure({ command, name, room, url, token, prompt: roomPrompt(name, room, roster), mcp, dataDir: stateDir() });
  for (const f of setup.files) fs.writeFileSync(f.file, f.content, { mode: 0o600 });
  const extra = [...setup.args];
  if (isClaude && mcp && !args.includes('--name') && !args.includes('-n')) extra.push('--name', name);
  if (mcp && setup.kind === 'other') {
    process.stderr.write(`metacom: ${path.basename(command)} is not a harness metacom configures (claude, codex, opencode), so it gets no mc_* tools; it is still wrapped, watched and typed into\n`);
  }
  trace(`harness ${setup.kind}${setup.tools ? ' with tools' : ''}`);
  ...
  const term = pty.spawn(command, [...args, ...extra], {
    ...
    rows,
    cwd,
    env: { ...cleanEnv, ...setup.env, TERM: 'xterm-256color', MC_AGENT: name, MC_ROOM: room, MC_URL: url, MC_TOKEN: token },
  });
  ...
  // changing when the agent is waiting for you, so stillness of the rendered screen is the
  // signal for a harness that emits no title or progress state.
  let lastChange = Date.now();
  let lastSample = Date.now();
  let shot = '';
  /// A repaint arrives as several chunks (clear, then the new frame), so the screen is only
  /// worth comparing once the burst is over: sample in the gaps. If the bytes never pause,
  /// the agent is streaming output, which is busy by definition.
  const sample = () => {
    const now = Date.now();
  ...
  // MARK: inbox. Commands and notes are typed when idle; control commands act at once. metacom
  ...
  const take = async (msg) => {
    ...
    seen.add(msg.id);
    // an encrypted room: the text arrives sealed, this device's key opens it
    const text = await mc.rooms.open(msg.room || room, msg.text);
    if (text === '[encrypted]') {
      trace(`cannot open ${msg.id.slice(0, 8)}: no key for room ${msg.room || room}`);
      return ack(msg);
    }
    msg = { ...msg, text };
  ...
  const flush = () => {
    ...
    // once; the screen-stillness heuristic needs a real quiet window.
  ...
  mc.api.agents.on('readRequest', async ({ id, lines }) => {
    trace(`readRequest ${id.slice(0, 8)} lines ${lines}`);
    // the screen goes back sealed under the room key when the room is encrypted
    try {
      const text = await mc.rooms.close(room, screen.lines(lines).join('\n'));
      await mc.api.agents.readReply({ id, text });
    } catch (e) {
      trace(`readReply failed: ${e.message}`);
    }
  });
