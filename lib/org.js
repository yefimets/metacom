...
class Org {
  constructor({ dataDir, auth, console }) {
    this.console = console;
    this.events = new EventEmitter();
    this.auth = auth;
    this.store = new Store(dataDir);
    this.media = new Media(dataDir);
    this.keys = new Keys(dataDir, console);
  ...
  bind(client, record, ip, publicKey = null) {
    const conn = { client, record, ip, publicKey, name: null, room: record.role === 'owner' ? '*' : null, calls: [] };
    if (publicKey) this.keys.touch(publicKey, { name: record.name, role: record.role });
  ...
  unbind(client) {
    ...
    if (member && member.connected) {
      ...
      member.lastSeen = now();
      this.console.log(`metacom: ${member.name} disconnected`);
      this.saveMembers();
  ...
  /// A connection for something living inside the org process (a connector): it calls
  /// say/send/read like a signed-in client, under the role it was given.
  ...
  register(conn, info = {}) {
    ...
    if (conn.publicKey) this.keys.touch(conn.publicKey, { name, role: conn.record.role });
    if (!this.byName.has(name)) this.byName.set(name, new Set());
    this.byName.get(name).add(conn.client);
    this.console.log(`metacom: ${name} (${kind}) joined room ${member.room} from ${conn.ip}`);
    this.saveMembers();
  ...
  setStatus(conn, status, reason = null) {
    ...
    member.lastSeen = now();
    this.console.log(`metacom: ${member.name} ${before} -> ${status}${member.reason ? ' (' + member.reason + ')' : ''}`);
  ...
  system(room, body) {
    const msg = { id: id(), ts: now(), room, kind: 'system', from: { name: 'metacom', role: 'system', kind: 'system' }, text: body };
  ...
  rooms() {
    ...
    for (const room of Object.keys(this.keys.state.rooms)) {
      if (!summary.has(room)) summary.set(room, { room, agents: 0, online: 0, working: 0, blocked: 0, attention: 0 });
    }
    return [...summary.values()].map((s) => ({ ...s, encrypted: this.keys.encrypted(s.room) })).sort((a, b) => a.room.localeCompare(b.room));
  }
  ...
  emit(client, event, data) {
    try {
      ...
      this.console.warn(`metacom: event ${event} failed: ${error.message}`);
    }
  }
...
module.exports = { Org, MAX_TEXT, NAME };
