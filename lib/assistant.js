...
${history || '(none)'}`;
  ...
  async execute(session, name, args) {
    const { conn } = session;
    try {
      // encrypted rooms: the server's own device key must have been granted the room
      const { keys, members } = this.org;
      if (name === 'message_agent') {
        const room = members.get(String(args.agent || ''))?.room || session.room;
        const r = await this.org.send(conn, args.agent, keys.close(room, args.text), 'command');
        return `${r.delivered ? 'delivered to' : 'queued for offline'} ${r.to}`;
      }
      if (name === 'read_agent') {
        const r = await this.org.read(conn, args.agent, args.lines || 40);
        const text = keys.open(members.get(r.name)?.room, r.text);
        if (text === null) return 'the screen is encrypted for a room this server was not granted';
        return text.trim() ? text : '(empty screen)';
      }
      if (name === 'say_to_room') {
        const msg = this.org.say(conn, session.room, keys.close(session.room, args.text));
        return `posted to room ${msg.room}`;
      }
      return 'refused: not a metacom tool';
    } catch (error) {
