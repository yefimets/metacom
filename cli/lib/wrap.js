...
const wrap = async ({ name, room, repo, caps, accept, command, args, config, mcp = true }) => {
  ...
  const term = pty.spawn(command, [...args, ...extra], {
    ...
    rows,
    cwd,
    env: { ...process.env, MC_AGENT: name, MC_ROOM: room, MC_HUB_URL: url, MC_TOKEN: token },
  });
  ...
  const classify = () => {
    ...
    const busy = signalled ? busySignal : Date.now() - lastOutput < 1500;
    if (busy) return update('working', signalled ? (progressSeen ? 'progress' : 'title') : 'output');
