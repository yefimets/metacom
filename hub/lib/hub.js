...
const text = (value, what = 'text') => {
  if (typeof value !== 'string' || value.trim().length === 0) throw fail(400, `${what} is required`);
...
class Hub {
  ...
  publicMember(m) {
    const { name, kind, room, repo, caps = [], host, command, status, connected, since, lastSeen, attention, reason } = m;
    return { name, kind, room, repo, caps, host, command, status, connected: Boolean(connected), since, lastSeen, attention: Boolean(attention), reason: reason || null };
  }

  saveMembers() {
    const list = [...this.members.values()].map(({ name, kind, tokenId, room, repo, caps, host, command, since, lastSeen }) => ({
      name, kind, tokenId, room, repo, caps, host, command, since, lastSeen,
    }));
  ...
  say(conn, room, body) {
    ...
    const msg = { id: id(), ts: now(), room: target, kind: 'say', from: this.from(conn), text: text(body) };
    this.store.appendRoom(target, msg);
    this.broadcast('room/message', msg, target);
    return msg;
  }

  async send(conn, to, body, kind = 'command', wait = null) {
    ...
    const result = { id: msg.id, to: member.name, delivered: clients.size > 0, queued: !clients.size };
  ...
  async dispatch(conn, body, room) {
    this.owner(conn);
    text(body);
    const members = this.list(conn, room);
    const pick = await route(body, members, { ...this.router, console: this.console });
    if (!pick) throw fail(404, 'No agent is connected');
    const result = await this.send(conn, pick.agent, body, 'command');
